...
> [!IMPORTANT]
> Приложение **не является VPN** - обойти геоблок (например, получить доступ к ChatGPT) при помощи ByeByeDPI **нельзя**!
...
_А если всё-таки очень хочется получить доступ к ИИ-сервисам и некоторым другим ресурсам с геоблоком - читайте про [специальный DNS](/docs/05-others/#проксирующие-dns)._
...
- В пункте **Фильтр приложений** выберите режим **белого списка** ([подробнее про списки](/docs/03-features/#whitelist-blacklist))
...
- Дождитесь окончания подбора. Об успешном окончании будет свидетельствовать строка ```Проверка завершена```. Стратегии сортируются по количеству ответивших доменов автоматически. Вылетает подбор - решение [здесь](/docs/04-problems/#crash-proxy-test).
...
Если в подборе стратегий не нашлось стратегии, которая бы давала доступ к вашему сервису, рассмотрите возможность [подбора пользовательских стратегий](/docs/03-features/#свой-список-команд-стратегий). Стратегии для него можно взять в [Telegram-группе ByeByeDPI](https://t.me/byebyedpi_group), или в других местах интернета.
