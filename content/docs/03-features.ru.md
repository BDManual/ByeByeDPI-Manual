...
Данный переключатель заставляет ByeByeDPI принудительно искать маршруты до сервера по новой версии IP (если вы собираетесь включать его, то убедитесь, что ваш оператор обслуживает IPv6 и сайты нужные вам имеют IPv6 адрес).
...
После выдачи прав вы можете указать у аргументов с поддержкой file (можете ознакомится с этим в [документации ByeDPI](https://github.com/hurfea/byedpi)) путь к файлу

Пример стратегии: `-f512 -t7 -l:'/storage/emulated/0/Download/g.bin'`. (Путь к файлу вы можете узнать через приложения по типу Cx Проводник.)
