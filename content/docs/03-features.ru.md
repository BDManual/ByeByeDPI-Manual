...
> [!IMPORTANT]
> Использовать свой список стратегий нужно **только в случае**, если в результате автоподбора небольшой процент ответа от доменов. Если вы видите, что у стратегий одинаковый (невысокий) процент ответа - решение [тут](/docs/04-problems/#у-команд-стратегий-одинаковый-невысокий-процент)
...
- Дождитесь окончания проверки и действуйте дальше как при [стандартной настройке](/docs/01-start/#настройка). Если подбор вылетает - решение [здесь](/docs/04-problems/#вылетает-подбор).
...
- После этого ищите стратегию [аналогичным образом](/docs/01-start/#setting), которым искали стратегию для YouTube.
...
`ToggleActivity` позволяет управлять ByeByeDPI через внешние приложения автоматизации, например `MacroDroid`.

Поддерживаются следующие параметры:

| Параметр      | Действие                                                                 | Тип    |
| ------------- | ------------------------------------------------------------------------ | ------ |
| `strategy`    | Обновить стратегию. Если сервис запущен - выполнить перезапуск для применения изменений | String |
| `only_update` | Только обновить аргументы запуска без изменения состояния сервиса         | Bool   |
| `only_start`  | Запустить сервис, если он не запущен                                     | Bool   |
| `only_stop`   | Остановить сервис, если он запущен                                       | Bool   |

Если не указаны параметры `only_update`, `only_start` или `only_stop`, активность работает как переключатель вкл/выкл.

> [!TIP]
> Это может быть полезно для автоматического переключения между наборами параметров при смене сети, например при переходе с Wi-Fi на мобильный интернет или между разными операторами связи.
...
Чтобы воспользоваться скриптом, скачайте [файл](https://github.com/BDManual/ByeByeDPI-Manual/blob/main/automatic_switching.category) и импортируйте его в MacroDroid.
...
> [!CAUTION]
> Рекомендуется данный переключатель оставить в положении по умолчанию. Потенциально он может (и будет) ломать подключения ко многим сайтам. В худшем сценарии - интернет на вашем устройстве перестанет работать вообще.

Данный переключатель заставляет ByeByeDPI устанавливать соединения по протоколу IPv6. Его поддерживают далеко не все провайдеры и сайты. Узнать, поддерживает ли ваш провайдер протокол IPv6 можно, например, на 2ip.ru. Также рекомендуется выполнить вручную DNS-запрос для необходимых доменов, чтобы убедиться, что они имеют IPv6 адреса (адреса формата fd80:dcif... У IPv6 адреса содержат буквы латинского алфавита).

## Использование файлов в стратегиях

Для использования файлов в стратегии нужно разрешить приложению работу с файлами (ByeByeDPI — Настройки — Доступ к файлам)

После выдачи прав можно указывать путь к файлу у аргументов, поддерживающих передачу файла (можете ознакомится с этим в [документации ByeDPI](https://github.com/hufrea/byedpi)).

Пример стратегии: `-f512 -t7 -l:'/storage/emulated/0/Download/g.bin'`. (Путь к файлу вы можете узнать из файлового менеджера системы, или поставив любой удобный из магазина приложений.).
