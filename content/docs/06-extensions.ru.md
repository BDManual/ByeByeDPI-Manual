...
Затем [передать](/docs/03-features//#my-list) уже изменённую стратегию в автоподбор.
...
Чтобы его использовать, необходимо указать ID (название, например Cloudflare из встроенных) списка доменов/подсетей из [настроек автоподбора](/docs/03-features//#domain-list). Можно создать свой список и указать его название. Получиться должно примерно так:
...
-j:"{list:subnets}"
```
Второе будет работать ТОЛЬКО если у вас в разделе [настроек доменов](/docs/03-features//#domain-list) прописан список из подсетей с названием `subnets`.
